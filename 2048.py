...
def initial():
    ...
    while count < 2:
        ...
        if board[row][col] == 0:
            ...
            board[row][col] = n
            count = count + 1
            textsurface = myfont.render(str(n), False, BLACK)
...
def gen_board():
    screen.fill(WHITE)
    for i in range (0,5):
        pygame.draw.line(screen, BLACK, (200,100*(i+1) ), (600, 100*(i+1)))
        pygame.draw.line(screen, BLACK, (200+100*i,100 ), (200+100*i,500 ))
    ...
    for row in range(0, 4):
        for col in range(0, 4):
            if board[row][col] != 0:
                textsurface = myfont.render(str(board[row][col]), False, BLACK)
                screen.blit(textsurface, (250+100*col, 150 + row*100))
    pass
...
def score_update():
    ...
    if score > highest_score:
        highest_score = score
    textsurface = myfont.render(str(highest_score), False, BLACK)
